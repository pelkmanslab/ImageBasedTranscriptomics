function localPath = nnpc(networkPath)
%NNPC is a network path conversion function mapping remote path to its local
%counterpart.
%
% It reflects a branching off from the main npc by Thomas.
...
if ~os.path.exists(localPath)
    fprintf('nnpc: Returning a non-existing local path "%s" \n', localPath);
end
...
if ispc
    ...
    %%% note that IP numbers might change, but that the name sever has
    %%% become inaccesible via UZH network %%%
